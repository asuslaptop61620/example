...
(18, 'Bitly url shortener', 'fa fa-cut', '#', 18, '18', '1', '1', '0', 0, '0'),
...
ALTER TABLE `ip_domain_info` ADD `organization` VARCHAR(100) NOT NULL AFTER `isp`;

UPDATE `menu` SET `only_admin` = '0' WHERE `menu`.`name` = "Google url shortener";
UPDATE `menu` SET `only_admin` = '0' WHERE `menu`.`name` = "Code minifier";

ALTER TABLE `email_config` ADD `smtp_type` ENUM('Default','tls','ssl') NOT NULL DEFAULT 'Default' AFTER `smtp_user`;

INSERT INTO `menu_child_2` (`id`, `name`, `url`, `serial`, `icon`, `module_access`, `parent_child`, `only_admin`, `only_member`, `is_external`) VALUES (NULL, 'Frontend Settings', 'admin_config/frontend_configuration', '1', 'fa fa-newspaper-o', '', '1', '1', '0', '0');
ALTER TABLE `config` ADD `bitly_login` VARCHAR(255) NOT NULL AFTER `virus_total_api`;
ALTER TABLE `config` ADD `bitly_api_key` VARCHAR(255) NOT NULL AFTER `bitly_login`;
ALTER TABLE `config` ADD `bitly_access_token` VARCHAR(255) NOT NULL AFTER `bitly_api_key`;
INSERT INTO `menu` ( `name`, `icon`, `url`, `serial`, `module_access`, `have_child`, `only_admin`, `only_member`, `add_ons_id`, `is_external`) VALUES ('Cron Job', 'fa fa-clock-o', 'cron_job/index', '23', '15', '0', '1', '0', '0', '0');

ALTER TABLE `config` ADD `facebook_app_id` VARCHAR(255) NOT NULL AFTER `bitly_access_token`, ADD `facebook_app_secret` VARCHAR(255) NOT NULL AFTER `facebook_app_id`;
ALTER TABLE `social_info` ADD `fb_comment_plugin` VARCHAR(255) NOT NULL AFTER `fb_comment`;
ALTER TABLE `web_common_info` ADD `fb_total_comment_plugin` VARCHAR(255) NOT NULL AFTER `fb_total_comment`;

ALTER TABLE `social_info` DROP `stumbleupon_view`;
ALTER TABLE `social_info` DROP `stumbleupon_like`;
ALTER TABLE `social_info` DROP `stumbleupon_comment`;
ALTER TABLE `social_info` DROP `stumbleupon_list`;