
<style>
/*	.info-box .progress .progress-bar {
	    background: #eee; ?>!important;
	}*/

	.info-box .progress {
	    background: #fff!important;
	   
	}
	.info-box-icon{

		background-color:#fff;border-right: 1px solid #eee;

	}
	.info-box-number{
		color: <?php echo $THEMECOLORCODE; ?>
	}
	.progress-description{
		font-size: 18px;
		color: <?php echo $THEMECOLORCODE; ?>
	}
</style>
<div role="tabpanel" class="tab-pane active" id="overview">
	<div id="overview_success_msg" class="text-center" ></div>	
	<!-- <div id="overview_name"></div> -->
	<!-- <div id="visitor_analysis_name"></div> -->
	<div class="row" style="margin-top: 20px;">
		

		<div class="col-xs-10 col-md-10 col-lg-10" style="margin-right: -23px;">
			<!-- Date range -->
			<div class="form-group">
				<label><?php echo $this->lang->line("date range");?>:</label>
				<div class="input-group">
					<div class="input-group-addon">
						<i class="fa fa-calendar"></i>
					</div>
					<input type="text" class="form-control pull-right reservation" id="overview_date" />
				</div><!-- /.input group -->
			</div><!-- /.form group -->
			<!-- end of date range -->
		</div>
		<div class="col-xs-12 col-lg-1" style="margin-top:25px;"><button class="btn btn-info search_button"><i class="fa fa-search"></i> <?php echo $this->lang->line("search");?></button></div>

		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
          <!-- LINE CHART -->
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title" style="color: blue; word-spacing: 4px;"><i class="fa fa-bar-chart"></i> <?php echo $this->lang->line('Day Wise New Visitor Report From'); ?> <span id="overview_from_date"></span> <?php echo $this->lang->line('to'); ?> <span id="overview_to_date"></span></h3>
              <div class="box-tools pull-right">
                <button class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                <button class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
              </div>
            </div>
            <div class="box-body chart-responsive">
              <div class="chart" id="line-chart" style="height: 300px;"></div>
            </div><!-- /.box-body -->
          </div><!-- /.box -->
        </div><!-- /.col lg-10 -->
		
		<div style="margin-top: 30px;" class="col-xs-10"></div>

        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
        	<div class="info-box">
				<span class="info-box-icon"><i class="fa fa-users blue"></i></span>
				<div class="info-box-content">
					<span class="info-box-text" style="font-size: 20px;"><?php echo $this->lang->line('Total Unique Visitor'); ?></span>
					<span class="info-box-number" id="total_unique_visitor"></span>
				</div><!-- /.info-box-content -->
			</div><!-- /.info-box -->
        </div>
        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
        	<div class="info-box">
				<span class="info-box-icon"><i class="fa fa-file-text-o orange"></i></span>
				<div class="info-box-content">
					<span class="info-box-text" style="font-size: 20px;"><?php echo $this->lang->line('Total Page View'); ?></span>
					<span class="info-box-number" id="total_page_view"></span>
				</div><!-- /.info-box-content -->
			</div><!-- /.info-box -->
        </div>

        <div style="margin-top: 20px;" class="col-xs-12"></div>

        <div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
        	<div class="info-box">
				<span class="info-box-icon"><i class="fa fa-clock-o blue"></i></span>
				<div class="info-box-content">
					<!-- <span class="info-box-text">Inventory</span> -->
					<span class="info-box-number" id="average_stay_time"></span>
					<div class="progress">
						<div class="progress-bar" style="width: 70%"></div>
					</div>
					<span class="info-box-text">
						<?php echo $this->lang->line('Average Stay Time'); ?>
				</div><!-- /.info-box-content -->
			</div><!-- /.info-box -->
        </div>

        <div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
        	<div class="info-box">
				<span class="info-box-icon"><i class="fa fa-eye blue2" style="color: #fff;"></i></span>
				<div class="info-box-content">
					<!-- <span class="info-box-text">Inventory</span> -->
					<span class="info-box-number" id="average_visit"></span>
					<div class="progress">
						<div class="progress-bar" style="width: 70%"></div>
					</div>
					<span class="info-box-text">
						<?php echo $this->lang->line('Average Visit'); ?>
					</span>
				</div><!-- /.info-box-content -->
			</div><!-- /.info-box -->
        </div>
        <div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
        	<div class="info-box">
				<span class="info-box-icon"><i class="fa fa-sign-out green"></i></span>
				<div class="info-box-content">
					<!-- <span class="info-box-text">Inventory</span> -->
					<span class="info-box-number" id="bounce_rate"></span>
					<div class="progress">
						<div class="progress-bar" style="width: 70%"></div>
					</div>
					<span class="info-box-text">
						<?php echo $this->lang->line('Bounce Rate'); ?>
					</span>
				</div><!-- /.info-box-content -->
			</div><!-- /.info-box -->
        </div>
	
	</div>

</div>