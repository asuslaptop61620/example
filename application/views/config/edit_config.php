<?php $this->load->view('admin/theme/message'); ?>
<?php 

if(array_key_exists(0,$config_data))
$google_safety_api=$config_data[0]["google_safety_api"]; 
else $google_safety_api="";

if(array_key_exists(0,$config_data))
$virus_total_api=$config_data[0]["virus_total_api"]; 
else $virus_total_api="";

if(array_key_exists(0,$config_data))
$moz_access_id=$config_data[0]["moz_access_id"]; 
else $moz_access_id="";

if(array_key_exists(0,$config_data))
$moz_secret_key=$config_data[0]["moz_secret_key"]; 
else $moz_secret_key="";

if(array_key_exists(0,$config_data))
$mobile_ready=$config_data[0]["mobile_ready_api_key"]; 
else $mobile_ready="";

if(array_key_exists(0,$config_data))
$bitly_login=$config_data[0]["bitly_login"]; 
else $bitly_login="";

if(array_key_exists(0,$config_data))
$bitly_api_key=$config_data[0]["bitly_api_key"]; 
else $bitly_api_key="";

if(array_key_exists(0,$config_data))
$bitly_access_token=$config_data[0]["bitly_access_token"]; 
else $bitly_access_token="";

if(array_key_exists(0,$config_data))
$facebook_app_id=$config_data[0]["facebook_app_id"]; 
else $facebook_app_id="";

if(array_key_exists(0,$config_data))
$facebook_app_secret=$config_data[0]["facebook_app_secret"]; 
else $facebook_app_secret="";

 ?>
<section class="content-header">
   <section class="content">
<!--    <?php if($this->session->userdata("user_type")=="Admin") { ?> <div class="alert alert-info text-center"><?php echo $this->lang->line("To know more read") ?> <a target="_BLANK" href="<?php echo site_url();?>documentation/#!/administration_settings_gt_connectivity_settings"> <?php echo $this->lang->line("documentation"); ?></a></div> <?php } ?> -->
     	<div class="box box-info custom_box">

		    	<div class="box-header">
		         <h3 class="box-title"><i class="fa fa-connectdevelop"></i> <?php echo $this->lang->line("connectivity settings");?></h3>
		        </div><!-- /.box-header -->
		       		<!-- form start -->

		    <form style="color:#607D8B !important" class="form-horizontal" enctype="multipart/form-data" action="<?php echo site_url().'config/edit_config';?>" method="POST">
		        <div class="box-body">		        	
		           	<div class="form-group">
		              	<label class="col-sm-3 control-label" for="">Google API  Key
		              	</label>
		                	<div class="col-sm-9 col-md-6 col-lg-6">
		               			<input name="google_safety_api" value="<?php echo $google_safety_api;?>"  class="form-control" type="text">		               
		             			<span class="red"><?php echo form_error('google_safety_api'); ?></span>
		             		</div>
		            </div>
		           <div class="form-group">
		             	<label class="col-sm-3 control-label" for="">MOZ Access ID
		             	</label>
	             		<div class="col-sm-9 col-md-6 col-lg-6">
	               			<input name="moz_access_id" value="<?php echo $moz_access_id;?>"  class="form-control" type="text">		          
	             			<span class="red"><?php echo form_error('moz_access_id'); ?></span>
	             		</div>
		           </div> 

		           <div class="form-group">
		             	<label class="col-sm-3 control-label" for="">MOZ Secret Key
		             	</label>
	             		<div class="col-sm-9 col-md-6 col-lg-6">
	               			<input name="moz_secret_key" value="<?php echo $moz_secret_key;?>"  class="form-control" type="text">		          
	             			<span class="red"><?php echo form_error('moz_secret_key'); ?></span>
	             		</div>
		           </div>  

		           <div class="form-group">
		             	<label class="col-sm-3 control-label" for="">VirusTotal Key
		             	</label>
	             		<div class="col-sm-9 col-md-6 col-lg-6">
	               			<input name="virus_total_api" value="<?php echo $virus_total_api;?>"  class="form-control" type="text">		          
	             			<span class="red"><?php echo form_error('virus_total_api'); ?></span>
	             		</div>
		           </div>  

	       	           <div class="form-group">
	       	             	<label class="col-sm-3 control-label" for="">Bitly Login
	       	             	</label>
	                    		<div class="col-sm-9 col-md-6 col-lg-6">
	                      			<input name="bitly_login" value="<?php echo $bitly_login;?>"  class="form-control" type="text">		          
	                    			<span class="red"><?php echo form_error('bitly_login'); ?></span>
	                    		</div>
	       	           </div>  

	           	           <div class="form-group">
	           	             	<label class="col-sm-3 control-label" for="">Bitly Api Key
	           	             	</label>
	                        		<div class="col-sm-9 col-md-6 col-lg-6">
	                          			<input name="bitly_api_key" value="<?php echo $bitly_api_key;?>"  class="form-control" type="text">		          
	                        			<span class="red"><?php echo form_error('bitly_api_key'); ?></span>
	                        		</div>
	           	           </div> 	           	          

	           	            <div class="form-group">
	           	             	<label class="col-sm-3 control-label" for="">Bitly Access Token
	           	             	</label>
	                        		<div class="col-sm-9 col-md-6 col-lg-6">
	                          			<input name="bitly_access_token" value="<?php echo $bitly_access_token;?>"  class="form-control" type="text">		          
	                        			<span class="red"><?php echo form_error('bitly_access_token'); ?></span>
	                        		</div>
	           	           </div>

   	           	            <div class="form-group">
   	           	             	<label class="col-sm-3 control-label" for=""> Facebook App ID
   	           	             	</label>
   	                        		<div class="col-sm-9 col-md-6 col-lg-6">
   	                          			<input name="facebook_app_id" value="<?php echo $facebook_app_id;?>"  class="form-control" type="text">		          
   	                        			<span class="red"><?php echo form_error('facebook_app_id'); ?></span>
   	                        		</div>
   	           	            </div>
            	            <div class="form-group">
            	             	<label class="col-sm-3 control-label" for=""> Facebook App Secret
            	             	</label>
                         		<div class="col-sm-9 col-md-6 col-lg-6">
                           			<input name="facebook_app_secret" value="<?php echo $facebook_app_secret;?>"  class="form-control" type="text">		          
                         			<span class="red"><?php echo form_error('facebook_app_secret'); ?></span>
                         		</div>
            	            </div>       
		                        
		           </div> <!-- /.box-body --> 

		           	<div class="box-footer">
		            	<div class="form-group">
		             		<div class="col-sm-12 text-center">
		               			<button name="submit" type="submit" class="btn btn-primary btn-lg"><i class="fa fa-save"></i> <?php echo $this->lang->line("Save");?></button>
               					<button type="button" onclick='goBack("config",1)' class="btn btn-default btn-lg"><i class="fa fa-close"></i><?php echo $this->lang->line("Cancel");?></button>  
		             		</div>
		           		</div>
		         	</div><!-- /.box-footer -->         
		        </div><!-- /.box-info -->       
		    </form>     
     	</div>
   </section>
</section>



