    ...
    */
    public function edit_config()
    {
        ...
        if ($_POST) 
        {
            ...
            $this->form_validation->set_rules('bitly_login',         '<b>Bitly Login</b>',                 'trim');
            $this->form_validation->set_rules('bitly_api_key',         '<b>Bitly Api Key</b>',                 'trim');
            $this->form_validation->set_rules('bitly_access_token',         '<b>Bitly Access Token</b>',                 'trim');
            $this->form_validation->set_rules('facebook_app_id',         '<b>Facebook App Id</b>',                 'trim');
            $this->form_validation->set_rules('facebook_app_secret',         '<b>Facebook App Secret</b>',                 'trim');
            ...
            else 
            {
                ...
                $bitly_login=addslashes(strip_tags($this->input->post('bitly_login', true)));
                $bitly_api_key=addslashes(strip_tags($this->input->post('bitly_api_key', true)));
                $bitly_access_token=addslashes(strip_tags($this->input->post('bitly_access_token', true)));
                $facebook_app_id=addslashes(strip_tags($this->input->post('facebook_app_id', true)));
                $facebook_app_secret=addslashes(strip_tags($this->input->post('facebook_app_secret', true)));

                $update_data=array("google_safety_api"=>$google_safety_api,"moz_access_id"=>$moz_access_id,"moz_secret_key"=>$moz_secret_key,"mobile_ready_api_key"=>$google_safety_api,"virus_total_api"=>$virus_total_api,"bitly_login"=>$bitly_login,"bitly_api_key"=>$bitly_api_key,"bitly_access_token"=>$bitly_access_token,"facebook_app_id"=>$facebook_app_id,"facebook_app_secret"=>$facebook_app_secret);

                $insert_data=array("google_safety_api"=>$google_safety_api,"moz_access_id"=>$moz_access_id,"moz_secret_key"=>$moz_secret_key,"mobile_ready_api_key"=>$google_safety_api,"virus_total_api"=>$virus_total_api,"bitly_login"=>$bitly_login,"bitly_api_key"=>$bitly_api_key,"bitly_access_token"=>$bitly_access_token,"facebook_app_id"=>$facebook_app_id,"facebook_app_secret"=>$facebook_app_secret,"user_id"=>$this->session->userdata("user_id"));
    ...
    public function admin_permission_display_crud($value, $row)
    {
        if ($value == "only me") 
        {
            return "<span class='label label-light'> <i class='fa fa-user green'></i> ".$this->lang->line('only me')."</sapn>";
        } 
        else 
        {
            return "<span class='label label-light'><i class='fa fa-users green'></i> ".$this->lang->line('everyone')."</sapn>";
        }
    }
